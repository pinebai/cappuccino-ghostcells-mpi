...
!     Logicals for oundary conditions on domain boundary faces
      ...
      ! Obstacles
      allocate(visob(nxyza),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: visob"

      if(iobst.eq.1) then

      allocate(ios(nobst),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: ios"
      allocate(ioe(nobst),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: ioe"
      allocate(jos(nobst),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: jos"
      allocate(joe(nobst),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: joe"
      allocate(kos(nobst),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: kos"
      allocate(koe(nobst),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: koe"

      allocate(dnow(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: dnow"
      allocate(dnoe(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: dnoe"
      allocate(dnon(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: dnon"
      allocate(dnos(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: dnos"
      allocate(dnob(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: dnob"
      allocate(dnot(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: dnot"

      allocate(genow(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: genow"
      allocate(genoe(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: genoe"
      allocate(genon(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: genon"
      allocate(genos(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: genos"
      allocate(genob(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: genob"
      allocate(genot(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: genot"

      allocate(suedow(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: suedow"
      allocate(suedoe(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: suedoe"
      allocate(suedon(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: suedon"
      allocate(suedos(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: suedos"
      allocate(suedob(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: suedob"
      allocate(suedot(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: suedot"

      allocate(typobw(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobw"
      allocate(typobe(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobe"
      allocate(typobn(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobn"
      allocate(typobs(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobs"
      allocate(typobb(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobb"
      allocate(typobt(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobt"

      allocate(typobtw(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobtw"
      allocate(typobte(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobte"
      allocate(typobtn(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobtn"
      allocate(typobts(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobts"
      allocate(typobtb(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobtb"
      allocate(typobtt(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobtt"

      allocate(typobcw(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobcw"
      allocate(typobce(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobce"
      allocate(typobcn(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobcn"
      allocate(typobcs(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobcs"
      allocate(typobcb(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobcb"
      allocate(typobct(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: typobct"

      allocate(tobwest(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: tobwest"
      allocate(tobeast(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: tobeast"
      allocate(tobnorth(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: tobnorth"
      allocate(tobsouth(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: tobsouth"
      allocate(tobbottom(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: tobbottom"
      allocate(tobtop(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: tobtop"

      allocate(cobwest(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: cobwest"
      allocate(cobeast(nobst,nyzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: cobeast"
      allocate(cobnorth(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: cobnorth"
      allocate(cobsouth(nobst,nxzo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: cobsouth"
      allocate(cobbottom(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: cobbottom"
      allocate(cobtop(nobst,nxyo),stat=ierr) 
        if(ierr /= 0)write(*,*)"allocation error: cobtop"
